...
MySQL 数据库安装完成后，自带了以下四个数据库：

| 数据库               | 含义                                                                                            |
| -------------------- | ----------------------------------------------------------------------------------------------- |
| `mysql`              | 存储 MySQL 服务器正常运行所需要的各种信息<br/>（时区、主从、用户、权限等）                      |
| `information_schema` | 提供了访问数据库元数据的各种表和视图<br/>包含数据库、表、字段类型及访问权限等                   |
| `performance_schema` | 为 MySQL 服务器运行时状态供了一个底层监控功能<br/>主要用于收集数据库服务器性能参数              |
| `sys`                | 包含了一系列方便 DBA 和开发人员利用<br/>`performance_schema` 性能数据库进行性能调优和诊断的视图 |
...
所以如果想要检查这些文本的文本格式，就会使用到mysqlbinlog日志管工具，日志文件在数据目录下的 binlog.00000x
