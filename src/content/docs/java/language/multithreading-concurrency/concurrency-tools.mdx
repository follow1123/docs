...
> [代码](https://github.com/follow1123/java-language-examples/tree/main/standard-library/src/main/java/org/example/juc/utils/CompletableFutureTest.java)
...
使用以 `Async` 结尾的方法，如果不指定线程池，后面执行不带 `Async` 操作的任务都会使用默认的 `ForkJoinPool`
...
> [代码](https://github.com/follow1123/java-language-examples/tree/main/standard-library/src/main/java/org/example/juc/utils/LockSupportTest.java)

和 Object 的 `wait()` / `notify()`、Condition 的`await()` / `signal()` 类似，都是用来挂起和唤醒线程，用于线程协作

:::note

LockSupport 不需要在同步代码块中使用

LockSupport 不需要遵循先等待后唤醒的步骤
