...
> [代码](https://github.com/follow1123/java-language-examples/tree/main/standard-library/src/main/java/org/example/juc/locks/ReentrantLockTest.java)
...
`StampedLock` 支持 `tryOptimisticRead()` 方法（乐观读）读取完毕后需要做一次戳校验如果校验通过，表示这期间确实没有写操作，数据可以安全使用，如果校验没通过，需要重新获取读锁，保证数据安全
