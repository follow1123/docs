...
> [代码](https://github.com/follow1123/java-language-examples/tree/main/syntax/src/main/java/org/example/multithreading/safety/syncd/method/SynchronizedMethodTest.java)

### 解决重复卖票问题

用实现 `Runnable` 接口的方式实现同步方法解决重复卖票问题
...
### 实现 `Runnable` 接口，并使用内部的属性当作同步监视器

<details>

<summary>
    实现 <code>Runnable</code> 接口的类
</summary>
...
用静态属性当作资源，并使用当前的 Class 当作同步监视器

<details>

<summary>
    继承 <code>Thread</code> 的类
</summary>
...
> [代码](https://github.com/follow1123/java-language-examples/tree/main/syntax/src/main/java/org/example/multithreading/safety/LockTest.java)

1. 创建 Lock 实例
2. 执行 `lock()` 方法，锁定对共享资源的调用
3. `unlock()` 方法，释放对共享资源的锁定
