...
在 Java 中，在声明方法时，当在完成方法功能时如果有未知的数据需要参与这些未知的数据需要在调用方法时才能确定，那么我们吧这样的数据通过形参表示

在方法体中，用这个形参名来代表那个未知的数据，而调用者在调用时，对应的传入实参就可以

在 JDK5.0 之前只能把元素类型设计为 Object, JDK5.0 时 Java 引入了 `参数化类型（Parameterized type）` 的概念，允许我们在创建集合时指定集合元素的类型

## Java内泛型的使用

> [代码](https://github.com/follow1123/java-language-examples/tree/main/syntax/src/main/java/org/example/generics/GenericsTest.java)
...
> [代码](https://github.com/follow1123/java-language-examples/tree/main/syntax/src/main/java/org/example/generics/wildcards/WildcardsTest.java)
...
由于所有引用类型的值都可以是 null，所以可以写入 null 值
