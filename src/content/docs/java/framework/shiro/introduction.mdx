...
- `principals` - 身份，一般对应用户名、邮箱
- `credentials` - 证明/凭证，一般对应密码

### Authorization（授权）

- Subject - 主体，对应用户等
- 资源 - 对应菜单，按钮等
- 权限 - 对应是否能对资源进行 crud 操作
- 角色 - 权限集合
...
2. `resources` 目录下新建 `shiro.ini`

```ini title="resources/shiro.ini"
[users]
admin=123456
```
